...
export default function Services() {
  ...
  const filtered = useMemo(() => {
    ...
    return SERVICES.filter(s => {
      ...
      const inSearch = !q ? true : (s.title + " " + s.desc).toLowerCase().includes(q);
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 pt-24 pb-14">
          ...
          <p className="mt-3 text-lg text-slate-600 max-w-3xl">
            From discovery to deployment — reliable, scalable software with clear communication.
          </p>
        </div>
      ...
      <section className="py-12">
        <div className="max-w-7xl mx-auto px-4 grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {filtered.map((s, i) => {
            const d = SERVICES_DETAILS[s.id] || { highlights: [], features: [], tech: [] };
            return (
              <article
                key={s.id}
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    ...
                    style={{ background:"rgba(6,208,1,.12)", color:"var(--green-dark)" }}
                  >
                    {s.icon}
                  </div>
                ...
                {d.highlights?.length ? (
                  <div className="mt-4 flex flex-wrap gap-2">
                    {d.highlights.map(h => (
                      <span key={h} className="px-2 py-1 rounded-md text-xs border border-emerald-200/60 bg-white text-emerald-700">
                        {h}
                      </span>
                    ))}
                  </div>
                ...
                {d.features?.length ? (
                  <ul className="mt-5 space-y-1.5 text-sm text-slate-700">
                    {d.features.slice(0, 5).map(f => <li key={f}>• {f}</li>)}
                  </ul>
                ) : null}
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6">
            <div className="grid md:grid-cols-5 gap-4 text-center">
              {[
                ...
                <div key={t} className="reveal" style={{ animationDelay: `${60 + i * 60}ms` }}>
      ...
      {/* ===== Big CTA ===== */}
      <section className="py-16" style={{ background: "linear-gradient(135deg, #e7fcef 0%, #b7f2d9 100%)" }}>
        <div className="max-w-7xl mx-auto px-4 text-center">
          <h3 className="text-3xl font-bold text-slate-900">Not sure where to start?</h3>
          <p className="mt-3 text-slate-700">Tell us your goal. We’ll propose a clear, step-by-step plan.</p>
          <Link
            to="/quote"
